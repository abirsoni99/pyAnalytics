...
#parameters include - begin,end, and number of rand required
help(np.random.random_integers)
#1-Dim
x1 = np.random.randint(10, size=6);
#here 10 is the max x1
x1.shape
x1
#2-Dim
x2 = np.random.randint( 10, size=(3,4)) ;x2
#3 rows, 4 column - 12 values
x2.shape

#3-Dim
x3 = np.random.randint(10, size=(3,4,5)) ; x3
#3 matrix of size 4*5
x3.shape

#Print those values
x0; x1;x2;x3
#use print() to print all independently
#%%%
#0-Dim
x0[0] ; x0[1]  #error
x0  #this will work
...
#2-Dim
x2 = np.random.randint( 10, size=(3,4)) ;x2
x2[0] #in multidimmension it means R,C
...
np.reshape(x, newshape=(-1,2)) #anyshape 2 col
np.reshape(x3[0],newshape=(-1,4))
#automatic reshaping
a = np.arange(30)
a
...
arr4  #note brackets
#arr5 = np.concatenate((arr3, np.array([1,2,3])), axis=1)
#arr5 
import numpy as np  
x=np.array([[1,2],[3,4]])  
x
